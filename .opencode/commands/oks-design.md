...
**本命令用途**：创建技术实现方案，包含数据库设计、API设计、组件设计、数据流设计等。
...
- ✅ 组件/模块设计和数据流
- ✅ 安全设计、性能设计、测试策略
- ❌ 不适用于：业务场景描述（用 `/oks-vision`）
- ❌ 不适用于：用户故事格式（用 `/oks-user-story`）
- ❌ 不适用于：编写代码实现（用 `/oks-implementation`）
...
1. **技术概述**: 2-3 句话说明技术实现思路
2. **数据库设计**: 定义表结构、字段类型、索引
3. **API 设计**: 定义接口路径、请求/响应格式
4. **数据流设计**: 描述核心流程和数据流转
5. **安全设计**: 认证授权、数据安全
6. **性能设计**: 性能目标和优化策略
7. **测试策略**: 单元测试、集成测试方案
...
1. **数据库设计**: 表结构是否合理？索引是否完整？
2. **API 设计**: 接口是否符合 RESTful 规范？
3. **数据流设计**: 流程是否清晰？边界情况是否处理？
4. **安全设计**: 是否有安全漏洞？
5. **性能设计**: 是否满足性能要求？
6. **技术风险**: 风险是否已评估？
```
